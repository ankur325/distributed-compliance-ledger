...
func (m MsgCreateValidator) ValidateBasic() error {
	...
	if !m.Address.Equals(sdk.ConsAddress(pubkey.Address())) {
		return errors.Wrap(errors.ErrInvalidRequest, "Validator Pubkey does not match to Validator Address")
	}

	if len(m.Description.Name) == 0 {
		return errors.Wrap(errors.ErrInvalidRequest, "Invalid Validator Name: it cannot be empty")
	}
