...
func ParseVID(str string) (uint16, error) {
	...
	if res == 0 {
		return 0, errors.Wrap(errors.ErrInvalidRequest, "Invalid VID: it must be non zero 16-bit unsigned integer")
	}
...
func ParsePID(str string) (uint16, error) {
	...
	if res == 0 {
		return 0, errors.Wrap(errors.ErrInvalidRequest, "Invalid PID: it must be non zero 16-bit unsigned integer")
	}
